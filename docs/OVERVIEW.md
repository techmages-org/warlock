...
- **`did:web`** identity: the deck signs as
  `did:web:decks.titaniumcomputing.com:warlock-cm5-01` under the org principal
  `did:web:titaniumcomputing.com`. The deck holds its own private key (file keystore,
  `0600`), generated once.
...
- A reference verifier confirms each record offline; the transparency-log host
  (`log.titaniumcomputing.com`) is kept on a *separate* subdomain from identity so a
